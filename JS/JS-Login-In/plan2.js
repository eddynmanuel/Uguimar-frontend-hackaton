//FUNCIONAMIENTO DE BOTONES DE PAGO - PLAN AVANZADO
paypal.Buttons({
    style: {
        color: 'blue',
        shape: 'pill',
    ...
    createOrder: function (data, actions) {
        return actions.order.create({
            purchase_units: [{
                description: 'Plan Avanzado - UGuimar',
                amount: {
                    currency_code: 'USD',
                    value: '94.00'
                }
    ...
    onApprove: function (data, actions) {
        return actions.order.capture().then(function (details) {
            ...
            localStorage.setItem('planActivo', 'Avanzado');
            localStorage.setItem('fechaSuscripcion', new Date().toISOString());

            alert('¡Pago completado exitosamente! Bienvenido al Plan Avanzado, ' + details.payer.name.given_name);
